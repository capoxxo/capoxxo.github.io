...
$(document).ready(() => {
  ...
  for (let i in links) {
    ...
    $('#marquee').append(`<a href="https://melon.rest/${link.link}" target="_BLANK">${link.name}</a>`);
...
$.getJSON(ipgeolocation, (data) => {
  writeLine(['Authenticating...', "brb hold on script runnin"], 30, () => {
    ...
    writeLine([`Access granted! <span style='font-size: 14px; color: #0f0;'>[success]</span>`, `omg welcom <i style='color: #0f0'>${ip}</i> uwu. btw, hiiii ${country} !!!`], 30, 500, () => {
      if (app.skippedIntro) return;
